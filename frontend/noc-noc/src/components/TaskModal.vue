        ...
        <div class="bg-gray-200 p-8 rounded-md flex">
            <div class="w-1/2 pr-4">
                ...
                <div>
                    ...
                    <button @click="submitComment" class="mt-4 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">Send comment</button>
                </div>
                    ...
                    <div class="overflow-y-auto max-h-52">
                        <ul>
                            <li v-for="comment in comments" :key="comment.id" class="mb-4">
                                ...
                                <span class="text-gray-500 text-sm ml-2">{{ formatTimestamp(comment.updated_at) }}</span>
                                </div>
            ...
            <div class="w-1/2 pl-4">
                <div class="flex mb-4">
                    ...
                    <template v-if="user.role === 'superadmin' || user.id === task.assigned_to">
                        <div class="relative">
                            <div @click="toggleDropdown" class="cursor-pointer bg-white border border-gray-300 rounded-md px-3">{{ selectedStatus }}
                            ...
                            <transition name="fade">
                                <ul v-if="dropdownOpen"
                                    ...
                                    <li v-for="status in states" :key="status"
                                        class="cursor-pointer px-4 py-2 hover:bg-gray-100" @click="selectStatus(status)">{{status }}</li>
                                </ul>
                            </transition>
                        </div>
                    </template>
                    <template v-else>
                        <span class="cursor-pointer bg-white border border-gray-300 rounded-md px-3">{{ selectedStatus }}</span>
                    </template>
                </div>
                <div class="flex mb-4">
                    ...
                    <template v-if="user.role === 'superadmin'">
                        <div class="relative">
                            <div @click="toggleDropdownUser" class="cursor-pointer bg-white border border-gray-300 rounded-md px-3">{{ selectedUser.name }}
                            ...
                            <transition name="fade">
                                <ul v-if="dropdownUserOpen"
                                    ...
                                    <li v-for="user in users" :key="user.id"
                                        class="cursor-pointer px-4 py-2 hover:bg-gray-100" @click="selectUser(user)">{{ user.name }}</li>
                                </ul>
                            </transition>
                        </div>
                    </template>
                    <template v-else>
                        <span class="cursor-pointer bg-white border border-gray-300 rounded-md px-3">{{ selectedUser.name }}></span>
                    </template>
                </div>
                <div>
                    <h3>Upload a file</h3>
                    <input type="file" @change="handleFileUpload">
                    <button @click="uploadFile" class="mt-3 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">Upload</button>
                </div>
                        ...
                        <ul>
                            <li v-for="file in files" :key="file.id" class="mb-4">
                                <div class="flex items-center mb-3">
                                    ...
                                    <span class="text-gray-500 text-sm ml-2">{{ formatTimestamp(file.updated_at) }}</span>
                                    <template v-if="user.role === 'superadmin' || user.id === file.user_id || user.id === task.assigned_to">
                                        <button class="bg-red-500 hover:bg-red-600 text-white px-1 rounded-md ml-2" @click="handleDeleteFile(file.id)"><font-awesome-icon icon="trash" /></button>
                                    </template>
                                </div>
                                <a :href="file.filepath" target="_blank" :title="file.filename" class="bg-gray-100 hover:bg-gray-200 p-2 rounded border border-gray-700 truncate">{{ file.filename }}</a>
                            </li>
                        </ul>
                    </div>
                </template>
                <template v-if="user.role === 'superadmin'">
                    <div class="text-end">
                        <button @click="handleDeleteTask" class="mt-4 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md"><font-awesome-icon icon="trash" /> Delete task</button>
                    </div>
                </template>
            </div>
            <div>
