...
<template>
  <div class="flex gap-4 overflow-x-auto">
    <div class="w-full border border-gray-700 rounded p-4">
      ...
      <ul class="h-[80%] min-h-40">
        <draggable class="space-y-2 h-full" v-model="pendingTasks" :group="{ name: 'tasks', put: true }"
          @change="handleDrop('Pending', $event)" :itemKey="task => task.id">
          <template v-slot:item="{ element: task }">
            <li class="border border-gray-600 rounded p-2 cursor-pointer transition duration-300 hover:bg-gray-300"
              :key="task.id" @click="openModal(task)">{{ task.title }}</li>
          </template>
        </draggable>
      </ul>
    </div>
    <div class="w-full border border-gray-500 rounded p-4">
      ...
      <ul class="h-[80%] min-h-40">
        <draggable class="space-y-2 h-full" v-model="inProgressTasks" :group="{ name: 'tasks', put: true }"
          @change="handleDrop('In progress', $event)" :itemKey="task => task.id">
          <template v-slot:item="{ element: task }">
            <li class="border border-gray-600 rounded p-2 cursor-pointer transition duration-300 hover:bg-gray-300"
              :key="task.id" @click="openModal(task)">{{ task.title }}</li>
          </template>
    ...
    <div class="w-full border border-gray-500 rounded p-4">
      ...
      <ul class="h-[80%] min-h-40">
        <draggable class="space-y-2 h-full" v-model="blockedTasks" :group="{ name: 'tasks', put: true }"
          @change="handleDrop('Blocked', $event)" :itemKey="task => task.id">
          <template v-slot:item="{ element: task }">
            <li class="border border-gray-600 rounded p-2 cursor-pointer transition duration-300 hover:bg-gray-300"
              :key="task.id" @click="openModal(task)">{{ task.title }}</li>
          </template>
    ...
    <div class="w-full border border-gray-500 rounded p-4">
      ...
      <ul class="h-[80%] min-h-40">
        <draggable class="space-y-2 h-full" v-model="completedTasks" :group="{ name: 'tasks', put: true }"
          @change="handleDrop('Completed', $event)" :itemKey="task => task.id">
          <template v-slot:item="{ element: task }">
            <li class="border border-gray-600 rounded p-2 cursor-pointer transition duration-300 hover:bg-gray-300"
              :key="task.id" @click="openModal(task)">{{ task.title }}</li>
          </template>
  ...
  <task-modal v-if="modalOpen" :task="selectedTask" @close="closeModal" @task-updated="taskUpdated" />
</template>